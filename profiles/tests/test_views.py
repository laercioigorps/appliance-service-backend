...
class TestCustomerView(TestCase):
    ...
    def test_list_3_customers_limited_by_10(self):
        ...
        response = client.get(
            "%s?limit=10&offset=0" % reverse("profiles:customer_list")
        )
    ...
    def test_list_3_customers_has_no_next_page(self):
        client = APIClient()
        client.force_authenticate(user=self.user2)
        response = client.get("%s?limit=3&offset=0" % reverse("profiles:customer_list"))
    ...
    def test_list_2_customers_has_next_page(self):
        client = APIClient()
        client.force_authenticate(user=self.user2)
    ...
    def test_list_customers_seconde_page_has_previous_page(self):
        client = APIClient()
        client.force_authenticate(user=self.user2)
